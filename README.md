...
- **Hugging Face**:
  ...
  3.  Paste it as `HF_TOKEN=your_token_here` in your `.env`.
...
**What it does:** Instantly loads the `v1.0.0.cbm` model into memory, translates a single patient JSON payload using BioBERT, and returns a `triage_acuity` prediction almost instantly.
**Offline Mode:** By default, it uses `local_files_only=True` to load BioBERT from your cache without checking the internet. If you need to download the model for the first time, set this to `False` in `ml/src/predict_api.py`.
